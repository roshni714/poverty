...
def get_optimal_knapsack_parameters(n_alpha_range, data_generator, original_cols, ntrain, nval, ntest, outcome, weight, density_estimation_params, savepath):
     """
    ...
    for trial in range(3):
        ...
        test_covariate_dataset = Dataset(test_df, outcome=None, weight=weight, covs=feature_list)
        test_dataset = Dataset(test_df, outcome=outcome, weight=weight, covs=feature_list)

        for n_alpha in n_alpha_range:
            res = tt.compute_auc(test_dataset=test_dataset, test_covariate_dataset=test_covariate_dataset, metrics=["post_transfer_poverty_rate"], budgets=BUDGETS)
            results.append({"n_alpha": n_alpha,
                            "trial": trial,
