...
from opt_targeted_transfers import RateTargetedTransfers, GapTargetedTransfers, BinaryRateTargetedTransfers, BinaryGapTargetedTransfers, write_result, split
...
def learn_continuous_rate(train_dataset, validation_dataset, test_covariate_dataset, test_dataset, continuous_rate_params, savepath):
    ...
    tt.fit(train_dataset, validation_dataset, n_knots=continuous_rate_params["density_estimation"]["n_knots"], 
...
def learn_binary_rate(train_dataset, validation_dataset, test_covariate_dataset, test_dataset, binary_rate_params, savepath):
    """
...
def main(config="hparam_results/output_gan_binary_gap.yaml", trainpath="data/train.parquet", testpath="data/test.parquet", savedir="learn_results"):
    ...
    train_dataset, validation_dataset, test_covariate_dataset, test_dataset = load_datasets(trainpath, testpath)
