...
class OptTargetedTransfers:
    ...
    def __init__(self, name="malawi_test"):
        self.name = name
    ...
    def run_opt(self, X_test, y_test, r_test=None, budget=0.1, c_bar=2.15, alpha_min=None, alpha_max=None, n_alpha=200):
    ...
    def evaluate(self, X_test, y_test, r_test=None, c_bar=2.15):
        if self.opt_policy is None:
            assert False, "Need to first run optimization"

        dataset = Dataset(X_test, y_test, r_test)
        result = post_transfer_metrics(dataset, self.opt_policy, c_bar)
        return result 
