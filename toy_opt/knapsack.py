...
def solve_fractional_knapsack_problem(xs, p_xs, convex_hulls, budget):
    ...
    for i in range(len(xs)):
        ...
        if len(convex_hulls[i]) > 1:
            ...
            ratio = (second_point[1] - init_point[1])/(second_point[0] - init_point[0])
            pq.put((ratio, (i, 1)))
    ...
    while total_spend < budget and pq.qsize() > 0:
        ...
        if total_spend > budget:
            ...
            c = np.minimum( (remainder/p_xs[x_idx] - curr_spend)/(prev_spend - curr_spend), 1.)
            
            assignments[x_idx] = [(convex_hulls[x_idx][hull_idx-1][1], c), (convex_hulls[x_idx][hull_idx][1], 1-c)]
