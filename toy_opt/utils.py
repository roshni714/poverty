...
def make_density_plot(true_cond_densities, estimated_cond_densities, title):
...
def make_estimated_density_plot(estimated_cond_densities, title):
    fig, ax = plt.subplots(1, 1, figsize=(6, 5))
    subset_cond = estimated_cond_densities[:10]
    a = np.linspace(0.0, max([1.5 * dist.mode for dist in subset_cond]), 1000)
    for i in range(10):
